...
class FloatLabelTextField: UITextField {
    ...
    fileprivate func setup() {
        ...
        if LSQAppearanceManager.sharedInstance.defaultViewBackgroundColor != nil {
            titleActiveTextColour = UIColor.white.withAlphaComponent(1.0)
            titleTextColour = UIColor.white.withAlphaComponent(0.8)
            textColor = UIColor.black.withAlphaComponent(0.6)
        }
        
        self.addSubview(title)
        
        if LSQAppearanceManager.sharedInstance.underlinedInputs {
            stroke = CAShapeLayer()
            let linePath = UIBezierPath()
            linePath.move(to: CGPoint(x: 0.0, y:self.frame.height))
            linePath.addLine(to: CGPoint(x: self.frame.width, y:self.frame.height))
            stroke?.path = linePath.cgPath
